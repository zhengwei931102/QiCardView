...
static CGFloat const Qi_SpringVelocity = 1.8f;
...
- (void)didPanStateEnded {
    ...
    if (self.currentPoint.x > self.maxRemoveDistance) {
        ...
        //zw修改
        CGFloat distanceY = endCenterX*self.currentPoint.y/self.currentPoint.x;//纵向移动距离
        CGPoint finishPoint = CGPointMake(endCenterX, distanceY+self.center.y);//目标center点
...
//            snapshotView.center = center;
    ...
    // zw新增下滑移除
        ...
        CGFloat endCenterY = [UIScreen mainScreen].bounds.size.height/2 + self.frame.size.height * 1.5;
        
        //zw修改
        CGFloat distanceX = endCenterY*self.currentPoint.x/self.currentPoint.y;//纵向移动距离
        CGPoint finishPoint = CGPointMake(distanceX+self.center.x, endCenterY);//目标center点
...
//            snapshotView.center = center;
    ...
    // zw新增上滑移除
        ...
        CGFloat endCenterY = -([UIScreen mainScreen].bounds.size.height/2 + self.frame.size.height);
...
- (void)restoreCellLocation {
    ...
    [UIView animateWithDuration:Qi_SpringDuration delay:0
         usingSpringWithDamping:Qi_SpringWithDamping
          initialSpringVelocity:Qi_SpringVelocity options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionAllowUserInteraction animations:^{
